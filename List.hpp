//
// Created by mikiele on 21.04.23.
//
...
void List<T, Allocator>::push_back_null() {
...
List<T, Allocator>::CommonIterator<IsConst>::operator--(int) {
  ...
  current_ = static_cast<Node*>(current_->prev);
  return tmp;
}
...
List<T, Allocator>::CommonIterator<IsConst>::operator++(int) {
  CommonIterator tmp = *this;
  current_ = static_cast<Node*>(current_->next);
  return tmp;
}
